// _worker.js - 合并了 index.html + functions/api 的全部功能
export default {
  async fetch(request) {
...
<head>
  ...
  <style>
      ...
      * {
      ...
      .menu-item:hover {
        ...
        border-bottom: 1px solid var(--menu-hover-border);
      }
      .menu-icon {
        width: 22px;
        height: 22px;
 ...
 <body>
    ...
    <nav class="floating-menu">
      ...
      <a href="https://github.com/chnbsdan/bing" class="menu-item" target="_blank">项目地址</a>
    </nav>
    ...
    <footer>
      ...
      <div style="margin-bottom: 0.5em">
        ...
        <a href="https://cn.bing.com" target="_blank" rel="nofollow noopener noreferrer">Bing_CN</a> |
        <a href="https://console.tencentcloud.com/edgeone/makers" target="_blank" rel="nofollow noopener noreferrer">EdgeOne</a>
      </div>
      <div>
        项目地址:
        <a href="https://github.com/chnbsdan/daily-image" target="_blank" rel="nofollow noopener noreferrer">daily-image</a>
        | MIT License © <span id="year"></span> |
        <a href="https://aoso.hangdn.com/" target="_blank" rel="nofollow noopener noreferrer">Hangdn Notes</a>
      </div>
      <script>
        document.getElementById("year").textContent = new Date().getFullYear();
        function updateTimes() {
          const startDate = new Date('2026-4-20');
          const now = new Date();
    ...
    <div class="modal" id="imageModal">
      <!-- 关闭按钮 - 左上角 -->
      <button class="modal-close" id="modalClose" aria-label="关闭预览">
        <i class="fas fa-times"></i>
      </button>
      
      <!-- 左右导航按钮 -->
      <button class="modal-nav modal-nav-left" id="navLeft" aria-label="上一张">
        <i class="fas fa-chevron-left"></i>
      </button>
      <button class="modal-nav modal-nav-right" id="navRight" aria-label="下一张">
      ...
      <div class="zoom-hint">🖱 滚动缩放 · 拖动平移</div>
    </div>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", () => {
        ...
        function openModal(index) {
          ...
          // 使用当前域名加载图片
          modalImage.src = `${window.location.origin}/webp/${item.startdate}.webp`;
        ...
        function goTo(delta) {
          const newIndex = (currentIndex + delta + imageData.length) % imageData.length;
          openModal(newIndex);
        }
        ...
        navLeft.addEventListener("click", (e) => { e.stopPropagation(); goTo(-1); });
        navRight.addEventListener("click", (e) => { e.stopPropagation(); goTo(1); });
        ...
        async function fetchImageData() {
          try {
            ...
            for (let i = 0; i < maxDays; i++) {
              ...
              const url = `https://raw.githubusercontent.com/chnbsdan/bing/main/json/${dateStr}.json`;
                  ...
                  if (data.images && data.images.length > 0) {
                    data.images.forEach(img => {
                      allImages.push({
                        ...
                        date: img.startdate.slice(0, 4) + '-' + img.startdate.slice(4, 6) + '-' + img.startdate.slice(6, 8),
            ...
            if (allImages.length === 0) {
              console.error("没有获取到任何图片数据");
              return;
            }

            // 按日期排序（最新的在前）
            allImages.sort((a, b) => b.startdate.localeCompare(a.startdate));
        ...
        function renderImages(data) {
          ...
          document.getElementById("hero-copy").textContent = today.copyright || '每日精选';
          // 使用当前域名加载图片
          loadImage(heroImg, `${window.location.origin}/webp/${today.startdate}.webp`);
          ...
          data.forEach((item, index) => {
            setTimeout(() => {
              ...
              // 使用当前域名加载图片
              loadImage(img, `${window.location.origin}/webp/${item.startdate}.webp`);
